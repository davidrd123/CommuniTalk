...
- Helper functions to be used in views are in `lib/view_helper_functions.rb`, and helper functions used in routes are in `lib/route_helper_functions.rb`.
- Code to interface with the database is in `lib/database_persistence.rb`.
- SQL code to create the database is in the `db` folder:
  - `db/schema.sql` contains SQL to create the database and populate it with sample data.
  - `db/schema_no_data.sql` contains SQL to create the database but not populate it with sample data.
  - `db/seed_data.sql` contains SQL to populate the database with sample data.
